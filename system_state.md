...
- 2026-06-18 - Maze Muncher center aisle was reopened with the verified 198-pellet maze. The ghost pen is now a compact row-10 house, ghost starts moved to y=10, player speed is 7.4, normal ghost base speed is 4.8, eaten eyes hold in the pen for 2.0s, and each life/level starts with a frozen 3-2-1 countdown plus respawn blinking while invulnerable.
...
- 2026-06-18 - Applied the Maze Muncher row-10 pen and countdown tuning in `PacMan.html`: replaced `MAZE` with the verified 21x21 / 198-pellet layout, moved all `ghostStarts` to y=10, changed player base speed to `7.4`, ghost base speed to `4.8`, raised `PEN_HOLD_TIME` to `2.0`, added `state.readyTimer`, froze actors/collisions during the 3-second countdown, and added respawn blink drawing. Verified script syntax, exact maze dimensions/rows, full pellet reachability, single-ghost eat isolation, eyes pen hold/revival, death reset timers, capped-`dt` center-aisle crossing, and speed constants with Node/VM checks.
